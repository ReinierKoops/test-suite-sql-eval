...
def print_and_return_scores(scores, etype, include_turn_acc=True):
    ...
    important_score = dict()

    if etype in ["all", "exec"]:
        # print ('======================= SEMANTIC MATCHING ACCURACY =========================')
        exec_scores = [scores[level]['exec'] for level in levels]
        # print_formated_s("--> semantic", exec_scores, '{:<20.3f}')
        
        important_score['semantic'] = exec_scores[-1]

    if etype in ["all", "match"]:
        # print ('\n======================= EXACT SET MATCHING ACCURACY ========================')
        exact_scores = [scores[level]['exact'] for level in levels]
        # print_formated_s("--> exact set match", exact_scores, '{:<20.3f}')
        
        important_score['exact_set'] = exact_scores[-1]
        
    return important_score


def retrieve_gold_list(gold_file):
...
def evaluate_string(gold_string, predict_string, db_dir, etype, kmaps):
    ...
    for i, (p, g) in enumerate(zip(plist, glist)):
        ...
        for idx, pg in enumerate(zip(p, g)):
            ...
            if etype in ["all", "exec"]:
                exec_score = eval_exec_match(db=db, p_str=p_str, g_str=g_str, plug_value=DISABLE_VALUE,
                                             keep_distinct=DISABLE_DISTINCT, progress_bar_for_each_datapoint=False)
                if exec_score:
    ...
    return print_and_return_scores(scores, etype, include_turn_acc=include_turn_acc)
